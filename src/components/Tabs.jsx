...
import './Tabs.css'; // Importing the CSS file
...
const Tabs = () => {
  ...
  const fetchTabContent = async (tabNumber) => {
    if (tabContent[tabNumber]) return; // Return if already fetched
  ...
  return (
    <div className="container">
      ...
      <div className="tab-content">
        <h3>Title {activeTab}</h3>
        <p>{tabContent[activeTab] || 'Loading...'}</p>
      </div>
    </div>
  );
};
